long long  expo(long long  a, long long  b, long long  mod=MOD) {long long  res = 1; while (b > 0) {if (b & 1)res = (res * a) % mod; a = (a * a) % mod; b = b >> 1;} return res;}



const int N=1e6+3;
long long  arr[N];
void ncr(long long  n){
    arr[0]=1;
    long long  i;
    for( i=1;i<=n;i++){
        arr[i]=((((n-i+1)*expo(i,MOD-2))%MOD)*arr[i-1])%MOD;
    }
}

//run ncr(n) and this will give an array of binomial coefficient
//ncr in log(n) time.
